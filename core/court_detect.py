...
class CourtDetector:
    def __init__(self, model_path: str = "models/court_kpRCNN.pth", device: str = 'cuda'):
    ...
    def setup_RCNN(self):
        self.__court_kpRCNN = torch.load(self.model_path, map_location=self.device)
        self.__court_kpRCNN.to(self.device).eval()

    def del_RCNN(self):
        del self.__court_kpRCNN

    def get_court_info(self, img: np.ndarray) -> Tuple[Optional[List], bool]:
        image = img.copy()
        self.mse = None
    ...
    def get_court_boundary_params(self) -> Optional[List[float]]:
        if not self.got_info or self.__court_info is None:
            return None
        return self.__court_info
    ...
    def draw_court(self, image: np.ndarray, mode: str = "auto") -> np.ndarray:
    ...
    def player_detection(self, outputs: dict) -> Tuple[bool, Optional[List[List[List[float]]]]]:
    ...
    def get_net_position(self) -> Optional[float]:
        if self.__court_info is not None:
            return self.__court_info[4]
        return None

    def get_partitioned_keypoints(self) -> Optional[List[List[int]]]:
        if not self.got_info or self.__multi_points is None:
            return None
        return self.__multi_points
