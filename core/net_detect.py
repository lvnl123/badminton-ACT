...
class NetDetector:
    def __init__(self, model_path: str = "models/net_kpRCNN.pth", device: str = 'cuda'):
    ...
    def setup_RCNN(self):
        self.__net_kpRCNN = torch.load(self.model_path, map_location=self.device)
        self.__net_kpRCNN.to(self.device).eval()

    def del_RCNN(self):
        del self.__net_kpRCNN

    def get_net_info(self, img: np.ndarray):
    ...
    def draw_net(self, image: np.ndarray, mode: str = "auto") -> np.ndarray:
